...
auth_tuple = []
if auth_password:
    auth_tuple.append(netius.SimpleAuth(password=auth_password))
if auth_addresses:
    auth_tuple.append(netius.AddressAuth(auth_addresses))
...
regex = ((re.compile(r".+/.well-known/acme-challenge/.+"), "http://172.17.0.1:8004"),)
auth = {"lugardajoia.com": auth_tuple, "www.lugardajoia.com": auth_tuple}
redirect = {"lugardajoia.com": "www.lugardajoia.com"}
server = netius.extra.ReverseProxyServer(
    hosts=hosts, regex=regex, auth=auth, redirect=redirect, reuse=False
)
server._ssl_contexts = netius.common.LetsEncryptDict(
    server, hosts.keys(), letse_path=letse_path
)
